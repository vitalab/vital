...
AUX_PREDICTION = Mapping[str, Mapping[str, np.ndarray]]
...
class CamusPredictionWriter(BasePredictionWriter):
    ...
    def write_on_batch_end(
        ...
        prediction: Union[Tensor, Tuple[Tensor, AUX_PREDICTION]],
        ...
        Args:
            ...
            prediction: Predicted segmentation, along with optional auxiliary predictions.
                ...
                The auxiliary predications are organised by: view -> group -> dataset, where the final value, i.e. the
                numpy array, is the data to create the dataset from.
        ...
        if isinstance(prediction, tuple):
            pred_view_seg, aux_predictions = prediction
        else:
        ...
        with h5py.File(pl_module.log_dir / "test.h5", "a") as dataset:
            ...
            for group, group_data in aux_predictions.items():
                data_group = view_group.require_group(group)
                for tag, data in group_data.items():
                    data_group.create_dataset(tag, data=data)
