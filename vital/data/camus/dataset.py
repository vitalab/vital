...
def get_segmentation_attributes(
    ...
    if Label.ENDO in labels:
        attrs.update(
            {
                CamusTags.endo_area: EchoMeasure.structure_area(segmentation, Label.ENDO.value),
                CamusTags.endo_base_width: EchoMeasure.endo_base_width(segmentation),
                CamusTags.endo_length: EchoMeasure.endo_length(segmentation),
                CamusTags.endo_orientation: EchoMeasure.structure_orientation(
                    segmentation, Label.ENDO.value, reference_orientation=90
                ),
            }
        )
    if Label.EPI in labels:
        attrs.update(
            {
                CamusTags.epi_area: EchoMeasure.structure_area(segmentation, Label.EPI.value),
                CamusTags.epi_center_x: EchoMeasure.structure_center(
                    segmentation, [Label.ENDO.value, Label.EPI.value], axis=1
                ),
                CamusTags.epi_center_y: EchoMeasure.structure_center(
                    segmentation, [Label.ENDO.value, Label.EPI.value], axis=0
                ),
            }
        )
