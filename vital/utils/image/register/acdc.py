...
class AcdcRegisteringTransformer(AffineRegisteringTransformer):
    """ Class that allows to register ACDC dataset's image/segmentation pairs by implementing algorithms for finding
    the affine registering parameters. These parameters are obtained based on the content of the segmentation maps and
    not the input MRI image. The goal of this registration is to shift the centroid of the left ventricle in the middle
    of the image and rotate the image so the centroid of the right ventricle is on the left of the centroid of the left
    ventricle.
    """
    ...
    def _compute_rotation_parameters(self, segmentation: np.ndarray) -> float:
        """ Computes the angle of the rotation to apply to align the segmentation so that the RV center of mass is
        ...
        """
        lv_myo_center = np.array(self._find_structure_center(segmentation, [Label.LV.value, Label.MYO.value]))
        rv_center = np.array(self._find_structure_center(segmentation, Label.RV.value,
                                                         default_center=tuple(lv_myo_center)))
