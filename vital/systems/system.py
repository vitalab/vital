...
class VitalSystem(pl.LightningModule, ABC):
    ...
    def log_images(self, title: str,
        ...
        for j, (ax_title, img) in enumerate(axes_content.items()):
            print(ax_title, img.shape)

        for i in range(num_images):
            fig, axes = plt.subplots(1, len(axes_content.keys()))
            if info is not None:
...
class SystemComputationMixin(VitalSystem, ABC):
    ...
    def __init__(self, train_log_kwargs: dict, val_log_kwargs: dict, **kwargs):
        super().__init__(train_log_kwargs=train_log_kwargs, val_log_kwargs=val_log_kwargs, **kwargs)
